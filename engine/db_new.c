...
int db_add(DB* self, Variant* key, Variant* value){
    ...
    return memtable_add(self->memtable, key, value);  ==> apo daskalo
	
	variable1 = memtable_add(self->memtable, key, value);
...
int db_get(DB* self, Variant* key, Variant* value)
{
    ...
    // Reader wants to enter the crital area //
	pthread_mutex_lock(&self->readcount_mutex);
    self->readcount++;
    //there is atleast one reader in the increased in critical area //
    //this ensure no writer can enter if there is even one reader //
	//we give preference to readers here //

    if (memtable_get(self->memtable->list, key, value) == 1){
        variable2 = 1;
		return 1;
	}else{    
		variable2 = sst_get(self->sst, key, value);
	...
	pthread_mutex_lock(&self->readcount_mutex);
    self->readcount--; 
    //there is no reader is left in critical area //
    if(self->readcount==0)							
    // send a sign to enter the writer//	
		pthread_mutex_unlock(&self->readwrite_mutex); 
    //reader leaves 
