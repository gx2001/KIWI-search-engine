...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
    ...
    pthread_mutex_init(&(self->readcount_mutex),NULL);		==> added
    self->readcount=0;*/
...
/*
*function which manage writes in database 
*with read-write sychronization
*we allow in this situation only one writer to enter into critical area and leave 
*!return an integer write value
*/
int db_add(DB* self, Variant* key, Variant* value){
	int write_value;
	
    //lock the critical area for write
	pthread_mutex_lock(&self->readwrite_mutex);
    //check for compactation this code is from previously here 
    ...
    //add the key and value in database
	write_value = memtable_add(self->memtable, key, value); 
    //leaves the critical area 
    pthread_mutex_unlock(&self->readwrite_mutex);		
    return write_value;
}
/*
*function manage read in database
*with read-write sychroniazation
*reader enter critical area to read in database 
*icrease by one a counter which count the readers 
*if a reader is the first and a writer comes need to stop it and not accept to write simuteniously in database
*we need to give permision to a lot of readers in database
*!returns an integer with the read value 
*/
int db_get(DB* self, Variant* key, Variant* value)
{
    ...
    //reader enter in critical area 
	pthread_mutex_lock(&self->readcount_mutex);
    //with this counter increase exlude the situation if a writer enter in critical area
    self->readcount++;
    //there is at least one reader //
    if (self->readcount == 1){
        pthread_mutex_lock(&self->readwrite_mutex);
    }
    // leave of critical area to allow others readers to run simuteliously
    pthread_mutex_unlock(&self->readcount_mutex);
    // check if need to search in memtable or in sst to find the value
    if (memtable_get(self->memtable->list, key, value) == 1){
        read_value = 1;
	}
    else{    
		read_value = sst_get(self->sst, key, value); 
    }
    // enter in critical to excecute read 
	pthread_mutex_lock(&self->readcount_mutex);
    self->readcount--; 
    if(self->readcount==0)//if counter=0 exit brcause there is no one to read 
		pthread_mutex_unlock(&self->readwrite_mutex);//allow writers to write(db_add())
    // exit critical area
    pthread_mutex_unlock(&self->readcount_mutex);
    return read_value;
}
